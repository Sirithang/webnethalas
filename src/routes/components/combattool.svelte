<script lang="ts" module>
    ...
    function changeEnemyHealth(enemy: EnemyData, amount: number) {
        enemy.currentHealth = Math.max(
            0,
            Math.min(enemy.maxHealth, enemy.currentHealth + amount),
        );
    }
                ...
                <div class="flex flex-col items-center">
                    {#each currentEnemies.current as e}
                        <InfoGauge
                            ...
                            label="Health"
                            height={4}
                            onchangefunc={(change: number) =>
                                changeEnemyHealth(e, change)}
                            onmaxchangedfunc={(newVal: number) => {
                                changeEnemyMaxHealth(e, newVal);
                            }}
                        />
                    {/each}
                ...
                <div class="h-fit">
                    {#if currentCombatStage.current == 0}
                        {#if !stealthAttempted.current}
                            <Text
                                ><b>Attempt Surprise:</b> Stealth vs Enemy Mind.</Text
                            >
                            <div class="flex flex-row">
                                <Button
                                    onclick={() => {
                                        initiativeWon.current = true;
                                        stealthWon.current = true;
                                        currentCombatStage.current = 1;
                                    }}>Succeded</Button
                                >
                                <Button
                                    onclick={() => {
                                        stealthAttempted.current = true;
                                        stealthWon.current = false;
                                    }}>Failed</Button
                                >
                            </div>
                        {/if}
                        <Text
                            ><b>Initiative Check:</b> Perception {stealthAttempted.current &&
                            !stealthWon.current
                                ? "(-20, stealth failed)"
                                : ""} vs Enemy Mind.</Text
                        >
                    ...
                    {:else if currentCombatStage.current == 1}
                        <div class="flex flex-row">
                            <Text class="grow"
                                >1st: {initiativeWon.current
                                    ? "Player"
                                    : "Enemies"}</Text
                            >
                            {#if stealthWon.current}
                                <Text class="grow">|</Text>
                                <Text class="grow font-bold"
                                    >Stealth Won:+20 on first attack</Text
                                >
                            {/if}
                        </div>
                        ...
                        <Tabs.Root value="melee" class="w-full">
                            ...
                            <Tabs.Content value="melee">
                                <Text
                                    >Melee → weapon skill vs (combat skill -
                                    weapon speed).</Text
                                >
                                <Text
                                    >&nbsp;&nbsp;&nbsp;&nbsp;<b>Win</b>: compute <Button
                                        variant="link"
                                        onclick={() => {
                                            popupStates.isDamageStepPopupShown = true;
                                        }}>Damage</Button
                                    >. On Critical, damage x2</Text
                                >
                                <Text
                                    >&nbsp;&nbsp;&nbsp;&nbsp;<b>Loose</b>:
                                    Defender roll on <Button
                                        variant="link"
                                        onclick={() => {
                                            popupStates.isDefensivePopupShown = true;
                                        }}>defensive table</Button
                                    ></Text
                                >
                                <Text
                                    >&nbsp;&nbsp;&nbsp;&nbsp;<b>Both</b> Fail: Defender
                                    suffer 1 damage</Text
                                >
                                <Text
                                    >&nbsp;&nbsp;&nbsp;&nbsp;<b
                                        >Attacker Critical Fail</b
                                    >
                                    roll on <Button
                                        variant="link"
                                        onclick={() => {
                            ...
                            <Tabs.Content value="magic"
                                ><Text
                                    >Magic → Spellward or Magic Resistance check
                                    from defense</Text
                                ></Tabs.Content
                            >
                        </Tabs.Root>
                        ...
                        <div>
                            <Button
                                onclick={() => {
                                    xpGranted(50);
                                    startNewFight();
                                }}>Regular Fight Won(+50XP)</Button
                            >
                            <Button
                                onclick={() => {
                                    xpGranted(200);
                                    startNewFight();
                                }}>Overseer Defeated(+200XP)</Button
                            >
